...
def main():
    ...
    parser.add_argument('--results_dir',
                        default=r"post_smm4h_21/ru_nodrug_train_subset/")
    ...
    for train_subset_dirname in os.listdir(results_dir):
        ...
        for name in os.listdir(subset_dir):
            if name.startswith('seed'):
                seed = name.split('_')[-1]
                # print(seed)
                evaluation_file_path = os.path.join(subset_dir, name, "scores.txt")
    ...
    print(results_df)
    print(results_df[results_df.train_size == '1000'])
    # print('--')
    #print("Mean:")
    avg_quality_df = results_df.groupby(
        by="train_size")[["dev_p", "dev_r", "dev_f", "test_p", "test_r", "test_f"]].mean()
    # print(avg_quality_df)
    #print(f"--\nStd:")
    std_quality_df = results_df.groupby(
        by="train_size")[["dev_p", "dev_r", "dev_f", "test_p", "test_r", "test_f"]].std()
    ...
    avg_quality_df.to_csv(mean_output_path, )
    std_quality_df.to_csv(std_output_path, )
